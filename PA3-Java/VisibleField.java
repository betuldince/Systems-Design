...
public class VisibleField {
   ...
   */     
   public void resetGameDisplay() {
      for(int i =0;i<vField.length;i++){
         for(int j=0;j<vField[0].length;j++){
            vField[i][j]= COVERED; 
         }
      }
      numGuess = getMineField().numMines();
      gameOver=false;
   }
   ...
   /**
      This code determines what would happen squares when the game finish with exploded mine
   
   */
   private void gameOverState(){
      // Guessed correctly mainField = true,  vField = -2 then yellow (dont change state)
      // Guessed incorrect  mainField = false  vField = -2 then X (INCORRECT_GUESS)
      // Not guessed mine  mainField = true  vField = -1 or -3 then black (MINE)
      for(int i = 0;i<mineField.numRows();i++ ){
         for(int j=0;j<mineField.numCols();j++ ){
            if(mineField.hasMine(i,j) && getStatus(i,j)== MINE_GUESS){
   ...
   /**
      This code determines if the user win the game. User win the game if all the non mine squares are opened
   ...
   */
    private boolean checkWin(){
       ...
       int numSquare = mineField.numRows()*mineField.numCols();
       if(counter== numSquare-mineField.numMines()){
   ...
   /**
      This code implements dfs algorithm for opening squares recursively starting with (row,col) position. And it changes state of square to             appropriate one i.e. numAdjacentMines. It also checks if the game is won in the recursive call.
      @param row of the square
      @param col of the square
      @return whether to continue opening squares.
   */
   private boolean openEmpty(int row, int col){
      if(row>=0 && col >=0){
         if(!mineField.inRange(row,col)||isUncovered(row,col)|| mineField.hasMine(row,col) || getStatus(row,col)==MINE_GUESS ){
            return false;    
         }
   ...
   /**
      In the win stuation, this code makes all the squares yellow as they contains mines. 
   */
